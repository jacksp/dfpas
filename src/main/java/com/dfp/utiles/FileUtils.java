...
public class FileUtils {
	...
	private static File compresssionImage(InputStream in, String sName, String sExtension) throws IOException {
		...
		// File("C:\\Users\\amfranco\\Downloads\\compress.jpg");
	...
	public static File stream2file(FileItemStream item, String sExtension) throws Exception {
		...
		else if (sExtension.toUpperCase().equals(StringKeys.PNG) || sExtension.toUpperCase().equals(StringKeys.GIF))
		...
		/*
		 * final File tempFile =
		 * File.createTempFile(FilenameUtils.getBaseName(sName) + "-", "." +
		 * sExtension); tempFile.deleteOnExit(); try { FileOutputStream out =
		 * new FileOutputStream(tempFile); IOUtils.copy(in, out); } catch
		 * (Exception e) { throw new Exception("Error al convertir el fichero::"
		 * +e.getMessage()); }
		 * 
		 * return tempFile;
		 */
